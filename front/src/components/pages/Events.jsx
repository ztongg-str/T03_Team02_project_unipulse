...
import { eventsAPI, streaksAPI, savedEventsAPI } from "../../services/api";
...
export default function Events() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [eventsRes, streaksRes] = await Promise.allSettled([
      ...
      if (streaksRes.status === "fulfilled" && streaksRes.value?.data?.length) {
        setStreak(streaksRes.value.data[0]);
      }
    } catch {
      setEvents([]);
    } finally {
      ...
      <section className="bg-[radial-gradient(102.62%_445.27%_at_100%_0%,#FFDCC0_0%,#FFF8F0_100%)] py-12">
        <div className="page-container flex gap-12 items-center">
          ...
          <div className="w-[616px] h-[346.5px] rounded-2xl bg-[linear-gradient(135deg,var(--orange),#ff6b6b)] border-4 border-white shadow-[0_10px_15px_-3px_rgba(0,0,0,0.1),0_4px_6px_-4px_rgba(0,0,0,0.1)] flex items-center justify-center text-white font-bold text-xl relative shrink-0">
            Campus Life
            {streak && (
              <div className="absolute -bottom-4 -left-4 bg-white border border-[var(--clam-shell)] rounded-2xl px-5 py-4 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.1)] flex items-center gap-3 min-w-[200px]">
<svg width="24" height="24" viewBox="0 0 24 24" fill="none" 
className="shrink-0">
                  <path d="M12 2l2.5 8.5H22l-6.5 5.5L18 23l-6-4.5L6 23l2.5-6.5L2 10.5h7.5L12 2z" fill="var(--wild-watermelon)" />
                </svg>
                <div>
                  <div className="text-xs font-bold text-wild-watermelon leading-4">
                    WEEKLY STREAK
                  </div>
                  <div className="text-lg text-zeus leading-7">
                    {streak.currentStreak} Events Joined
                  </div>
                </div>
              </div>
            )}
