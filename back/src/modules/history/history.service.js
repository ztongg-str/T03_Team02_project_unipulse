...
export const markAttendance = async (registrationId, organizerId, attended) => {
  ...
  if (attended) {
    const { leveledUp } = await usersRepository.addXp(registration.userId, xpEarned);
    if (leveledUp) {
      await checkAndGrant(registration.userId, 'level_5');
      await checkAndGrant(registration.userId, 'level_10');
    }
